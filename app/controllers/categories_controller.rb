class CategoriesController < ApplicationController
  ...
  def destroy
    if Post.where(category_id: @category.id).size == 0
      @category.destroy
      ...
      respond_to do |format|
        format.html { redirect_to admin_category_path(@category), notice: 'It is forbidden to delete categories associated with posts' }
        format.json { head :no_content }
      end
    end
  end
